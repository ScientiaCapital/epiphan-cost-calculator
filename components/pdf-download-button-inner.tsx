...
export function PdfDownloadButtonInner({ inputs, results }: Props) {
  ...
  return (
    <PDFDownloadLink
      ...
      className="mt-3 w-full py-3 px-4 bg-ink text-white font-semibold text-[14px] rounded-lg hover:bg-ink-deep transition-colors cursor-pointer flex items-center justify-center gap-2 no-underline"
    >
      {({ loading }) =>
