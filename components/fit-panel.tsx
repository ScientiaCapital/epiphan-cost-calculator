"use client";

import type { CalculatorInputs, CalculatorResults } from "@/lib/calculator";
import { getVerticalConfig } from "@/lib/verticals";
import { AE_BOOKING_URL } from "@/lib/constants";

interface FitPanelProps {
  inputs: CalculatorInputs;
  results: CalculatorResults;
}

/** Results view for "fit" verticals (Live Events, Broadcast): show the
 *  recommended portfolio + discovery angles, plus the one illustrative per-unit
 *  figure where we have it. No invented annual totals — that's the guardrail. */
export function FitPanel({ inputs, results }: FitPanelProps) {
  const cfg = getVerticalConfig(results.vertical);
  const fit = cfg.portfolioFit;

  return (
    <section aria-label={`${cfg.label} recommended fit`} className="mb-5">
      <div className="text-center p-5 bg-ink rounded-[10px] mb-5">
        <div className="text-[12px] font-bold text-green uppercase tracking-widest">
          {cfg.label} · {results.framing === "greenfield" ? "New build / per-project" : "Current setup"}
        </div>
        <div className="text-[20px] font-extrabold text-white leading-snug my-2">
          Let&rsquo;s scope the right setup for your {inputs.rooms.toLocaleString()}{" "}
          {cfg.labels.unitsLabel.toLowerCase()}
        </div>
        <div className="text-[13px] text-indigo-soft">
          A tailored cost model for this vertical is in the works &mdash; here&rsquo;s the fit and the
          questions that get us there.
        </div>
      </div>

      <div className="bg-white rounded-lg border-2 border-green p-4 mb-4">
        <h3 className="text-[14px] font-bold text-ink mb-2">&#9989; Recommended fit</h3>
        <ul className="space-y-1 mb-2">
          {fit.hardware.map((h) => (
            <li key={h} className="text-[13px] text-ink-2">
              <span className="font-semibold text-ink">{h}</span>
            </li>
          ))}
          {fit.software.map((s) => (
            <li key={s} className="text-[13px] text-teal">{s}</li>
          ))}
        </ul>
        <p className="text-[13px] text-ink-3 italic">{fit.note}</p>
      </div>

      <div className="bg-white rounded-lg border-2 border-teal p-4 mb-4">
        <h3 className="text-[14px] font-bold text-teal mb-2">What to quantify next</h3>
        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1">
          {cfg.discoveryFocus.map((d) => (
            <li key={d} className="text-[13px] text-ink-2 flex gap-1.5">
              <span className="text-teal">&bull;</span> {d}
            </li>
          ))}
        </ul>
      </div>

      <div className="bg-surface-2 rounded-lg p-4 mb-4">
        <h3 className="text-[12px] font-bold uppercase tracking-wide text-ink-3 mb-1.5">
          Who to bring into the room
        </h3>
        <p className="text-[13px] text-ink-2">
          <span className="font-semibold">Decision-maker:</span> {cfg.personas.atl.join(" · ")}
        </p>
        <p className="text-[13px] text-ink-2">
          <span className="font-semibold">Champion:</span> {cfg.personas.btl.join(" · ")}
        </p>
      </div>

      <a
        href={AE_BOOKING_URL}
        target="_blank"
        rel="noopener noreferrer"
        className="inline-flex items-center gap-1.5 py-2.5 px-4 bg-teal text-white font-semibold text-[13px] rounded-lg hover:bg-teal-deep transition-colors"
      >
        Talk to an Epiphan AE &rarr;
      </a>
    </section>
  );
}
