...
export function CostBreakdown({ inputs, results: r }: CostBreakdownProps) {
  ...
  return (
    <div className="bg-white rounded-[10px] shadow-[0_1px_4px_rgba(0,0,0,0.08)] overflow-hidden mb-5">
      <div className="px-5 py-4 font-bold text-[14px] uppercase tracking-wide border-b border-line flex items-center gap-2">
        &#128200; Cost Breakdown
      </div>

      {/* CMS end-of-life trigger — the #1 buying signal for AV/Media and Instructional Tech teams */}
      <div className="px-5 py-3 bg-ink text-white">
        <div className="text-[13px] font-semibold">
          Running an aging or end-of-life lecture-capture system? Here is what waiting costs you each year.
        </div>
        <div className="text-[12px] text-indigo-soft mt-0.5">
      ...
      <div>
        ...
        <div className="px-5 pt-4 pb-2 bg-surface-2 border-b border-line">
        ...
        <CostCategory id="cat-manual" confidence="asserted" name="Rooms per Tech: Manual Operation" cost={r.staffCost} barColor="amber" maxCost={maxCost}>
          ...
          (<strong className="text-ink-1">{formatCurrency(r.staffCost)}</strong>/yr) for projects instead of room-by-room firefighting.
          <br />
        ...
        <CostCategory id="cat-captures" confidence="calibrated" name="Failed & Missed Captures" cost={r.missedCaptureCost} barColor="red" maxCost={maxCost}>
          ...
          affecting about <strong className="text-ink-1">{r.studentsAffected.toLocaleString()}</strong> student-views. Each incident runs about <strong className="text-ink-1">$150</strong> in IT investigation, instructor coordination, and complaint handling.
          <br />
        ...
        <CostCategory id="cat-downtime" confidence="calibrated" name="Classroom Downtime" cost={r.downtimeCost} barColor="amber" maxCost={maxCost}>
          ...
          Aging rooms average <strong className="text-ink-1">{r.downtimeEventsPerRoom.toFixed(1)}</strong> disruptive failures/room/year versus under 0.5 for cloud-managed systems,
          which is <strong className="text-ink-1">{r.totalDowntimeEvents.toLocaleString()}</strong> disrupted sessions across your campus a year.
          <br />
        ...
        <CostCategory id="cat-retention" confidence="estimated" name="Student Retention: At-Risk Revenue" cost={r.retentionCost} barColor="blue" maxCost={maxCost}>
          ...
          whose experience was degraded enough to contribute to leaving represents <strong className="text-ink-1">{formatCurrency(r.retentionCost)}</strong> in at-risk tuition revenue.
          <br />
